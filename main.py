...
def _validate_symbol(symbol: str) -> bool:
    ...
    return not any(f in symbol.upper() for f in forbidden)
...
# ===================== FIXED CONFIGURATION (بدون تكرار) =====================
CONFIG = {
    # Exchange
    "EXCHANGE": "mexc",
    "QUOTE": "/USDT",
...
def price_acceptance_gate_5m(
    ...
    if ob:
        ...
        avg_vol = df_5m['volume'].iloc[-20:-1].mean()
        if last['volume'] < avg_vol * 0.7:
            return False, "ob_volume_too_low"
...
def analyze_market_structure(df: pd.DataFrame) -> Optional[MarketStructure]:
    ...
    try:
        ...
        liquidity_high = safe_float(df['high'].iloc[-20:].max())
        liquidity_low = safe_float(df['low'].iloc[-20:].min())
...
def analyze_volume_profile(df: pd.DataFrame, precheck_score: float = 0.0) -> Optional[VolumeProfileData]:
    if not CONFIG["ENABLE_VOLUME_PROFILE"] or df is None or len(df) < 50:
        return None
...
def detect_liquidity_grab(df: pd.DataFrame) -> Optional[LiquidityGrab]:
    ...
    try:
        ...
        support = safe_float(df['low'].iloc[-20:].min())
        resistance = safe_float(df['high'].iloc[-20:].max())
        current_atr = safe_float(df['atr'].iloc[-1]) if 'atr' in df.columns else 0
...
async def analyze_multi_timeframe(exchange, symbol: str) -> Optional[Dict]:
    try:
        ...
        volume_profile = analyze_volume_profile(df_15m)
        liquidity_grab = detect_liquidity_grab(df_5m)
...
def calculate_risk_levels(
    ...
    if not validate_price(entry) or entry <= 0:
        return 0.0, 0.0, 0.0, 0.0, position_size_usdt
    ...
    if market_structure.order_block:
        ob_sl = market_structure.order_block['low'] * 0.995
        if ob_sl <= 0 or ob_sl >= entry:
            ob_sl = 0.0
    
    atr_sl = entry - (atr * CONFIG["ATR_SL_MULT"])
    
    sl = 0.0
    
    if ob_sl > 0 and atr_sl > 0 and ob_sl < entry and atr_sl < entry:
        sl = max(ob_sl, atr_sl)
        logger.info(f"[Risk] Using combined SL: OB={ob_sl:.6f}, ATR={atr_sl:.6f}, Final={sl:.6f}")
    elif ob_sl > 0 and ob_sl < entry:
        sl = ob_sl
    elif atr_sl > 0 and atr_sl < entry:
        sl = atr_sl
    else:
        fallback_sl = entry * 0.95
        if fallback_sl <= 0 or fallback_sl >= entry:
            logger.error(f"[Risk] Invalid fallback SL: {fallback_sl:.6f} from entry {entry:.6f}")
            return 0.0, 0.0, 0.0, 0.0, position_size_usdt
        sl = fallback_sl
        logger.warning(f"[Risk] Using fallback SL: {sl:.6f} (5% from entry)")
    
    if sl <= 0 or sl >= entry:
        logger.error(f"[Risk] Invalid final SL: {sl:.6f} (entry: {entry:.6f})")
        return 0.0, 0.0, 0.0, 0.0, position_size_usdt
    ...
    if sl < hard_sl:
        logger.warning(f"[Risk] SL {sl:.6f} exceeds MAX_SL_PCT {CONFIG['MAX_SL_PCT']}%, clamping to {hard_sl:.6f}")
        sl = hard_sl
    
    initial_risk_pct = abs(entry - sl) / entry * 100
    adjusted_position_size = position_size_usdt
    
    if initial_risk_pct > CONFIG["MAX_SL_PCT"]:
        logger.warning(f"[Risk] Initial SL {initial_risk_pct:.2f}% > max {CONFIG['MAX_SL_PCT']}%. Reducing position size.")
        reduction_factor = CONFIG["MAX_SL_PCT"] / initial_risk_pct
        adjusted_position_size = position_size_usdt * reduction_factor
    
    risk = entry - sl
    
    if risk <= 0:
        logger.error(f"[Risk] Invalid risk calculation: {risk:.6f} (entry: {entry:.6f}, sl: {sl:.6f})")
        return 0.0, 0.0, 0.0, 0.0, position_size_usdt
    ...
    if not all(validate_price(p) and p > entry for p in [tp1, tp2, tp3]):
        logger.error(f"[Risk] Invalid TP levels: TP1={tp1:.6f}, TP2={tp2:.6f}, TP3={tp3:.6f}")
        return 0.0, 0.0, 0.0, 0.0, position_size_usdt
    
    rr_ratio = (tp3 - entry) / risk
    if rr_ratio < 2.0:
        logger.warning(f"[Risk] RR ratio too low: {rr_ratio:.2f}")
    
    return sl, tp1, tp2, tp3, adjusted_position_size
...
def should_run_order_flow(symbol: str, mtf_alignment: int, precheck_score: float, loop_count: int) -> bool:
    ...
    if mtf_alignment >= 2 and CONFIG.get("ORDER_FLOW_SAMPLING_ENABLED", True):
        n = max(1, int(CONFIG.get("ORDER_FLOW_SAMPLE_EVERY_N_LOOPS", 3)))
        symbol_offset = sum(ord(c) for c in symbol) % n
        return (loop_count + symbol_offset) % n == 0
        ...
        try:
            ...
            for symbol, trade_snapshot in emergency_trades:
                try:
                    ...
                    if sell_order:
                        risk = trade_snapshot.entry - trade_snapshot.original_sl
                        r_multiple = (current_price - trade_snapshot.entry) / risk if risk > 0 else 0
                        
                        await close_trade_full(symbol, current_price, "EMERGENCY_RECOVERY", exchange=exchange)
...
async def generate_quantum_signal(exchange, symbol: str) -> Optional[QuantumSignal]:
    try:
        ...
        structure_15m = mtf['structure_15m']
        structure_5m = mtf['structure_5m']
        volume_profile = mtf['volume_profile']
        liquidity_grab = mtf['liquidity_grab']
        df_15m = mtf['df_15m']
        df_5m = mtf['df_5m']
        ...
        position_size_usdt, position_size_pct = calculate_position_size(entry, structure_15m.order_block['low'] if structure_15m.order_block else entry - (atr * CONFIG["ATR_SL_MULT"]))
        if position_size_usdt == 0:
            return None
        ...
        sl, tp1, tp2, tp3, adj_position_size = calculate_risk_levels(
            entry, structure_15m, atr, liquidity_grab, position_size_usdt
        )
        
        if sl >= entry or not validate_price(sl):
            return None
        
        if adj_position_size != position_size_usdt:
            position_size_usdt = adj_position_size
            position_size_pct = (position_size_usdt / CONFIG["ACCOUNT_SIZE_USDT"]) * 100
...
async def _monitor_active_trades_internal(exchange):
    ...
    for symbol in symbols:
        try:
            ...
            elif current_price <= trade.current_sl:
                await close_trade_full(symbol, current_price, "SL")
                continue
...
• الدخول: {trade.entry:.6f}
• الخروج (سعر): {exit_price:.6f}
• الربح: {profit_pct:+.2f}%
• مضاعف R: {r_multiple:.2f}R
• نسبة الخروج: {exit_pct*100:.0f}%
• R المحقق: {exit_r:.2f}R
• المتبقي: {(current_remaining*100):.0f}%
{"• LIVE Sell: ✅" if is_live_trading_enabled() and live_sell_ok else ("• LIVE Sell: ❌" if is_live_trading_enabled() else "")}
"""
    ...
    if current_remaining <= 0.01:
        await close_trade_full(symbol, exit_price, "ALL_TPS", exchange=exchange)

# ===================== INSTITUTIONAL CLOSE TRADE FULL =====================
async def close_trade_full(symbol: str, exit_price: float, exit_type: str, exchange=None):
    trade = None
    remaining = 0
    fill_amount = 0
    try:
    ...
    if exit_type == "SL":
        STATS["trades_lost"] += 1
        STATS["total_r_multiple"] += total_r
    else:
        STATS["trades_won"] += 1
        STATS["total_r_multiple"] += total_r
    
    record_daily_r(total_r)
    
    db_manager.record_trade_history(symbol, trade, exit_price, exit_type, total_r, execution_mode)
...
"""
    ...
    logger.info(f"[Trade Closed] {symbol} - {exit_type} - {profit_pct:+.2f}% - {total_r:.2f}R")
...
async def toggle_computational_features():
    ...
    CONFIG["ORDER_FLOW_ENABLED"] = bool(CONFIG.get("ENABLE_ORDER_FLOW_BASELINE", False)) and (loop_count % 3 == 0)
    CONFIG["ENABLE_VOLUME_PROFILE"] = bool(CONFIG.get("ENABLE_VOLUME_PROFILE_BASELINE", False)) and (loop_count % 5 == 0)
...
async def async_main():
    ...
    try:
        ...
        # تم إزالة كل ما يتعلق بـ Colab (nest_asyncio, keep_alive)
        ...
        try:
            if exchange is not None:
                try:
                    ...
                    logger.error(f"❌ خطأ في إغلاق البورصة: {e}")
...
def main():
    ...
    while True:
        try:
            # إعادة تعيين علامة الإيقاف لكل تشغيل جديد
            shutdown_manager.should_stop = False
            asyncio.run(async_main())
        except KeyboardInterrupt:
            logger.info("\n👋 توقف البوت بواسطة المستخدم")
